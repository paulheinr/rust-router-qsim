...
fn read_network(path: String) -> IONetwork {
    IONetwork::from_file(&path)
}

fn convert_network(mut network: IONetwork) -> RoutingKitNetwork {
    ...
    for node in network.nodes() {
        ...
        first_out.push((first_out.len() + links_last_node) as EdgeId);
...
fn run(path: String) -> RoutingKitNetwork {
    convert_network(read_network(path))
}

//checks whether network consists of unique node ids
fn check_network_valid(network: IONetwork) {
    let mut node_ids: Vec<str> = network.nodes().iter().map(|n| n.id).collect();
    node_ids.dedup();
